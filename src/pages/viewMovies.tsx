...
export default function ViewMovie() {
  ...
  const fetchMovieData = async () => {
    await ApiServices.get(`/movie/${movie_id}`)
      .then((data) => setMovieDetails(data))
      .catch(() => {
        throw new Error("Unable to fetch Individuals details");
      });

    await ApiServices.get(`/movie/${movie_id}/images`)
      .then((data) => setMovieImages(data.backdrops))
      .catch(() => {
        throw new Error("unable to fetch movie Images");
      });

    await ApiServices.get(`/movie/${movie_id}/credits?language=en-US`)
      .then((data) => {
        setCastAndCrew(data);
      })
      .catch(() => {
        throw new Error("Unable to cast Images");
      });

    await ApiServices.get(`/movie/${movie_id}/videos`)
  ...
  const getTrailer = (videos: any) => {
    if (videos.length > 0) {
      trailervideo_id = videos.filter((item: any) => {
        if (item.type === "Trailer" && item.official) return item;
      });
    }
  };
  getTrailer(videos);
  ...
  const getDirector = () => {
    let directors: any = [];
    directors = castAndCrew?.crew.filter(
      (groupdept: any) => groupdept.job === "Director"
    );
            ...
            <>
              ...
              <div className="md:h-[360px] lg:h-[420px] flex flex-col md:flex-row items-center gap-3 mb-4">
                ...
                <div className="relative w-full md:grow h-[320px] md:h-full">
                  <iframe
                    className="rounded-xl absolute w-full h-full inset-0"
                    src={`https://www.youtube.com/embed/${trailervideo_id[0]?.key}?autoplay=1&mute=1`}
                  ></iframe>
                </div>
              </div>
              ...
              <ul className="m-0 py-4">
                <li className="list-none text-sm border-b-[1px] py-3 pt-0">
                ...
                <li className="list-none text-sm border-b-[1px] py-3 ">
                  <div className="flex flex-wrap gap-3 items-center">
                    {movieDetails?.genres?.map((genre: any) => (
                      <span
                        key={genre.id}
                        className="bg-primary_3 rounded-xl px-3 py-1.5 text-sm text-white"
                      >
                        {genre.name}
                      </span>
                    ))}
                ...
                <li className="list-none text-sm border-b-[1px] py-3">
                  Directed By : <strong>{castAndCrew?.crew ? getDirector() : "-"}</strong>
                </li>
                <li className="list-none text-sm border-b-[1px] py-3">
                  Duration : <strong> {getRuntime(movieDetails.runtime)} </strong>
                </li>
                <li className="list-none text-sm border-b-[1px] py-3">
                  <Link
                ...
                {movieImages.map((item: any, index: number) => {
                  if (index < 10) {
                    return (
                      <div
                        key={index}
                        className="relative w-34 h-32 rounded-xl overflow-hidden"
                      >
                        <img
                                    ...
                                    <div key={index} className='flex items-center gap-2'>
                                        {
                                            item.profile_path !== null ? <div className='w-20 h-20 rounded-full overflow-hidden'>
                                                <img src={`https://image.tmdb.org/t/p/original/${item.profile_path}`} alt={`${item.original_name}`} width={80}
                                                    height={80}
                                                    style={{ objectFit: 'cover' }} />
                                            </div>
                                                :
                                                <div className='w-20 h-20 rounded-full overflow-hidden bg-white grid place-content-center'>
                                                    <img src={`/profile_avatar.png`} alt={`${item.original_name}`} width={60}
                                                        height={60}
                                                        style={{ objectFit: 'cover' }} />
                                                </div>
                                        }
